...
export function useAppNavigation(state: AppState, actions: AppActions) {
  ...
  const navigateToScreen = useCallback((screen: Screen, tab?: string) => {
    console.log('📱 navigateToScreen вызван:', { screen, tab, currentScreen });
  ...
  const handleTabChange = useCallback((tab: string) => {
    console.log('🔄 handleTabChange вызван с tab:', tab);
    setActiveTab(tab);
    ...
    if (user?.role === 'seller') {
      ...
      switch (tab) {
        case 'home':
          ...
          break;
        case 'feed':
          console.log('🎬 Навигация: переход на видео-ленту');
          navigateToScreen('videoFeed', tab);
          break;
        case 'cart':
          navigateToScreen('cart', tab);
          break;
        case 'wallet':
          console.log('💰 Навигация: переход в кошелёк');
          navigateToScreen('wallet', tab);
          break;
        case 'profile':
  ...
  const navigateToTracking = useCallback((orderId: string) => {
    setSelectedOrderId(orderId);
