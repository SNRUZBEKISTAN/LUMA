...
export function useAppHandlers(state: AppState, actions: AppActions, navigation: any) {
  ...
  const { setCurrentScreen, setActiveTab, setUser, setPhoneNumber, setSelectedRole, setSellerId, setRegisterStep, setSelectedStoreId, setPromoData } = actions;
  const { handleTabChange, navigateToScreen } = navigation;
