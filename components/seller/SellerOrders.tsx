...
export function SellerOrders({ activeTab, onTabChange, onBack, onNavigate, onOrderClick, state, actions }: SellerOrdersProps) {
  const [selectedOrderTab, setSelectedOrderTab] = useState('all');
  const [searchQuery, setSearchQuery] = useState('');

  // Используем реальные данные заказов или мок-данные
  const sellerOrders = state?.sellerOrders || mockOrders;
  const currentStoreId = state?.selectedStoreId;
  
  // Получаем счетчик новых заказов для текущего магазина
  const newOrdersCount = actions?.getSellerNewOrdersCount ? 
    actions.getSellerNewOrdersCount(currentStoreId) : 
